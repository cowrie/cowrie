...
class _ShellChannel(SSHChannel):
    ...
    @ivar _creator: See L{__init__}
    @ivar _protocolFactory:  See L{__init__}
    @ivar _shellConnected:  See L{__init__}
    @ivar _protocol: An L{IProtocol} provider created using C{_protocolFactory}
    ...
    def openFailed(self, reason):
        """
        When the request to open a new channel to open this shell fails,
        fire the C{shellConnected} deferred with a failure indicating that.
        """
    ...
    def _shellFailure(self, reason):
        """
        When the request to open a shell in this channel fails, fire the
        C{shellConnected} deferred with a failure indicating this.
    ...
    def _shellSuccess(self, ignored):
        ...
        Also fire C{shellConnected} with the created protocol after it is
        connected to its transport.
        ...
        self._protocol = self._protocolFactory.buildProtocol(
            SSHShellAddress(
                ...
                self.conn.transport.creator.username))
        self._protocol.makeConnection(self)
        self._shellConnected.callback(self._protocol)


    def dataReceived(self, data):
        """
        When the shell's stdout data arrives over the channel, deliver it to
        the protocol instance.
...
class SSHShellClientEndpoint(object):
    ...
    def connect(self, protocolFactory):
        ...
        d.addCallback(self._shellCommand, protocolFactory)
        return d


    def _shellCommand(self, connection, protocolFactory):
        """
