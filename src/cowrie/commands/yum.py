...
class command_yum(HoneyPotCommand):
    ...
    def do_version(self):
        ...
        randhash = hashlib.sha1(b'%d' % randnum).hexdigest()
        randhash2 = hashlib.sha1(b'%d' % randnum2).hexdigest()
        yield self.sleep(1,2)
        self.write('Installed: 7/%s  %d:%s\n' % (arch,random.randint(500,800),randhash))
        self.write('Group-Installed: yum 13:%s\n' % randhash2)
        self.write('version\n')
        self.exit()
        return
...
Options:
  ...
  Plugin Options:
    ...
    def lineReceived(self, line):
      ...
      i = 1
      for p in packages:
        self.write('  Installing : %s-%s-%s.%s \t\t\t\t %d/%d \n' % (p,packages[p]['version'],packages[p]['release'],arch,i,len(packages)))
        yield self.sleep(0.5, 1)
        i += 1
      i = 1
      for p in packages:
        self.write('  Verifying : %s-%s-%s.%s \t\t\t\t %d/%d \n' % (p,packages[p]['version'],packages[p]['release'],arch,i,len(packages)))
        yield self.sleep(0.5, 1)
        i += 1
      self.write('\n')
      self.write('Installed:\n')
      for p in packages:
        self.write('  %s.%s %d:%s-%s \t\t' % (p,arch,random.randint(0,2),packages[p]['version'],packages[p]['release']))
      self.write('\n')                                                                     
      self.write('Complete!\n')
      self.exit()
