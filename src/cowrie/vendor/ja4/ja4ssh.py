...
def update_ssh_entry(entry, x, ssh_sample_count, debug_stream=None):
    ...
    if x["stream"] == debug_stream:
        print(
            f"stats[{len(entry['stats'])}]:tcp flag = {x['flags']}, c{e['client_packets']}s{e['server_packets']}_c{e['client_acks']}s{e['server_acks']}"
        )
...
def to_ja4ssh(x):
    ...
    if e["client_payloads"] or e["server_payloads"]:
        mode_client = (
            max(e["client_payloads"], key=e["client_payloads"].count)
            if e["client_payloads"]
            else 0
        )
        mode_server = (
            max(e["server_payloads"], key=e["server_payloads"].count)
            if e["server_payloads"]
            else 0
        )
