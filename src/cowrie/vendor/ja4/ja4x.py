...
def to_ja4x(x, debug_stream=-1):
    ...
    if "printable_certs" in x:
        ...
        for _i, _s in zip(issuers, subjects):
            ...
            try:
                ...
                cache_update(
                    x, f"JA4X.{idx}._Issuer", x[f"JA4X.{idx}._Issuer"], debug_stream
                )
            ...
            try:
                ...
                cache_update(
                    x, f"JA4X.{idx}._Subject", x[f"JA4X.{idx}._Subject"], debug_stream
                )
            except Exception as e:
                pass
    ...
    for idx, i in enumerate(x["extension_lengths"]):
        ...
        x[f"JA4X.{idx+1}"] = (
            f'{x["issuer_hashes"][idx]}_{x["subject_hashes"][idx]}_'
            + sha256(",".join(hex_strings).encode("utf8")).hexdigest()[:12]
        )
