...
def to_ja4(x, debug_stream):
    ...
    ext_len = "{:02d}".format(
        min(len([x for x in x["extensions"] if x not in GREASE_TABLE]), 99)
    )
    ...
    if x["signature_algorithms"] == "":
        ...
        x["sorted_extensions"] = (
            x["sorted_extensions"] + "_" + ",".join(x["signature_algorithms"])
        )
        x["original_extensions"] = (
            x["original_extensions"] + "_" + ",".join(x["signature_algorithms"])
        )
    ...
    x["original_ciphers"], cipher_len, original_ciphers = get_hex_sorted(
        x, "ciphers", sort=False
    )
    ...
    x[f"JA4_o.{idx}"] = (
        f"{ptype}{version}{sni}{cipher_len}{ext_len}{alpn}_{original_ciphers}_{original_extensions}"
    )
    x[f"JA4_r.{idx}"] = (
        f"{ptype}{version}{sni}{cipher_len}{ext_len}{alpn}_{x['sorted_ciphers']}_{x['sorted_extensions']}"
    )
    x[f"JA4_ro.{idx}"] = (
        f"{ptype}{version}{sni}{cipher_len}{ext_len}{alpn}_{x['original_ciphers']}_{x['original_extensions']}"
    )
    [
        cache_update(x, key, x[key], debug_stream)
        for key in [
            "domain",
            f"JA4.{idx}",
            f"JA4_r.{idx}",
...
def layer_update(x, pkt, layer):
    ...
    if layer == "quic":
        ...
        l = (
            pkt["layers"].pop(layer, None)
            if layer != "x509af"
            else pkt["layers"].pop("tls", None)
        )
    ...
    if layer == "x509af" and l:
        [
            x.update({key: l[f"tls_tls_{item}"]})
            for key, item in keymap["tls"].items()
            if f"tls_tls_{item}" in l
        ]
        (
            x.update({"issuer_sequence": l["x509if_x509if_rdnSequence"]})
            if "x509if_x509if_rdnSequence" in l
            else None
        )
        ...
        (
            ...
            if "x509sat_x509sat_printableString" in l
            else None
        )
...
def main():
    ...
    parser.add_argument(
        "-key", required=False, help="The key file to use for decryption"
    )
    ...
    parser.add_argument(
        "--ja4", action="store_true", default=False, help="Output JA4 fingerprints only"
    )
    parser.add_argument(
        "--ja4s",
        action="store_true",
        default=False,
        help="Output JA4S fingerprints only",
    )
    parser.add_argument(
        "--ja4l",
        action="store_true",
        default=False,
        help="Output JA4L-C/S fingerprints only",
    )
    parser.add_argument(
        "--ja4h",
        action="store_true",
        default=False,
        help="Output JA4H fingerprints only",
    )
    parser.add_argument(
        "--ja4x",
        action="store_true",
        default=False,
        help="Output JA4X fingerprints only",
    )
    parser.add_argument(
        "--ja4ssh",
        action="store_true",
        default=False,
        help="Output JA4SSH fingerprints only",
    )
    parser.add_argument(
        "-r",
        "--raw_fingerprint",
    ...
    parser.add_argument(
        ...
        required=False,
        action="store_true",
        help="Output original rendering",
    )
    parser.add_argument(
        "-f",
        "--output",
        nargs="?",
        const="ja4.output",
        help="Send output to file <filename>",
    )
    parser.add_argument(
        "-s",
        "--stream",
        nargs="?",
        const="0",
        help="Inspect a specific stream <stream>",
    )
    ...
    if args.key:
        ps = Popen(
            [
                ...
                args.pcap,
                "-o",
                f"tls.keylog_file:{os.path.abspath(args.key)}",
                "-T",
                "ek",
                "-n",
            ],
    ...
    for idx, line in enumerate(iter(ps.stdout.readline, "")):  # enumerate(sys.stdin):
        if "layers" in line:
            ...
            (
                ...
                if "ipv6" not in x["protos"]
                else layer_update(x, pkt, "ipv6")
            )
            ...
            if "tcp" in x["protos"] and "ja4ssh" in output_types:
                if (int(x["srcport"]) == 22) or (int(x["dstport"]) == 22):
                    ...
                    if (
                        "flags" in x
                        and int(x["flags"], 0) & TCP_FLAGS["FIN"]
                        and int(x["flags"], 0) & TCP_FLAGS["ACK"]
                    ):
