...
from common import cache_update, sha_encode
...
def to_ja4h(x, debug_stream=-1):
    ...
    x["headers"] = [
        ...
        for h in x["headers"]
        if not h.startswith(":")
        and not h.lower().startswith("cookie")
        and h.lower() != "referer"
        and h
    ]
    ...
    if "cookies" in x:
        if isinstance(x["cookies"], list):
            x["cookie_fields"] = [
                y.split("=")[0].lstrip().rstrip() for y in x["cookies"]
            ]
            x["cookie_values"] = [y.lstrip().rstrip() for y in x["cookies"]]
        else:
            x["cookie_fields"] = [
                y.split("=")[0].lstrip().rstrip() for y in x["cookies"].split(";")
            ]
    ...
    x["JA4H"] = (
        f'{method}{version}{cookie}{referer}{header_len}{lang}_{headers}_{cookies if len(cookies) else ""}_{cookie_values}'
    )
    x["JA4H_r"] = (
        f"{method}{version}{cookie}{referer}{header_len}{lang}_{','.join(raw_headers)}_"
    )
    x["JA4H_ro"] = (
        f"{method}{version}{cookie}{referer}{header_len}{lang}_{','.join(raw_headers)}_"
    )
    if "cookie_fields" in x:
        x[
            "JA4H_ro"
        ] += f"{','.join(unsorted_cookie_fields)}_{','.join(unsorted_cookie_values)}"
