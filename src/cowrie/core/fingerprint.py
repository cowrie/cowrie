...
# GREASE values (used in TLS to prevent ossification)
GREASE_VALUES = {
...
def sha_encode(values: list[str] | str) -> str:
    """
    Create a truncated SHA256 hash of the input.

    Args:
        values: Either a list of strings to join with commas, or a single string
    ...
    if isinstance(values, list):
        ...
        data = values
    return sha256(data.encode('utf8')).hexdigest()[:12]
    ...
    def get_tcp_info(self) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                self.ttl = struct.unpack('i', ttl_data)[0] if isinstance(ttl_data, bytes) else ttl_data
                info['ttl'] = self.ttl
...
class JA4HFingerprint:
    ...
    def generate(
        ...
        """
        # Method (first 2 chars)
        method_str = method[:2].lower()
        ...
        # Cookie presence
        cookie_str = '1' if cookies else '0'

        # Referer presence
        referer_str = '1' if referer else '0'

        # Language
        if accept_language:
            # Extract first language code
            lang = accept_language.split(',')[0].split('-')[0][:2]
        else:
            lang = '00'

        # Create header hash (sorted)
        sorted_headers = sorted([h.lower() for h in headers])
        ...
        # Format: {method}{version}{header_count}{cookie}{referer}{lang}_{header_hash}_{cookie_hash}
        return f"{method_str}{ver_str}{header_count:02d}{cookie_str}{referer_str}{lang}_{header_hash}_{cookie_hash}"


# Convenience functions for common use cases

def fingerprint_tcp_connection(transport: Any) -> Optional[str]:
    """
...
def fingerprint_ssh_kex(kex_algorithms: list[str], encryption_algorithms: list[str],
                        mac_algorithms: list[str], compression_algorithms: list[str]) -> str:
    """
